...
- 방금 하려던 답장 전체를 자연스러운 의미 단위로 나눠 messages 배열에 순서대로 담으세요.
- 보통 1~2개면 충분하고, 할 말이 많을 때만 최대 3개까지 사용하세요.
- 각 조각은 그 자체로 의미가 통하는 완전한 문장이나 표현이어야 합니다. 조사나 단어 하나만 남기는 식으로
  어색하게 쪼개지 마세요.
- 조각으로 나눈다고 해서 대화 내용, 말투, 다른 지침이 달라지는 것은 아닙니다. 같은 답장을 메신저처럼
  나눠 보내는 것뿐이니, 다른 모든 지침(1:1 대화, 질문 회피 금지, 질문/공감 마무리 균형 등)은 그대로 지키세요."""
